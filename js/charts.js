...
function renderEquityChart() {
  ...
  const values = state.equityCurve.map(d => d.cumulative);
  ...
  state.charts.equity = new Chart(ctx, {
    type: 'line',
    data: {
      labels: state.equityCurve.map(d => new Date(d.date).toLocaleDateString('fr-FR', { day: '2-digit', month: 'short' })),
      datasets: [{ data: values, borderColor: isPos ? '#10b981' : '#ef4444', backgroundColor: grad, borderWidth: 2, fill: true, tension: 0.4, pointRadius: 0, pointHoverRadius: 5 }]
    },
...
function renderMonthlyChart() {
  ...
  state.charts.monthly = new Chart(el.getContext('2d'), {
    ...
    data: {
      ...
      datasets: [{ data: last12.map(m => m.pnl), backgroundColor: last12.map(m => m.pnl >= 0 ? 'rgba(16,185,129,.7)' : 'rgba(239,68,68,.7)'), borderColor: last12.map(m => m.pnl >= 0 ? '#10b981' : '#ef4444'), borderWidth: 1, borderRadius: 6 }]
    },
...
function renderWinLossChart() {
  ...
  if (!el || !state.stats?.totalTrades) return;
  const s = state.stats;
  state.charts.winloss = new Chart(el.getContext('2d'), {
    type: 'doughnut',
    data: { labels: ['Wins', 'Losses', 'Break Even'], datasets: [{ data: [s.wins, s.losses, s.bes], backgroundColor: ['rgba(16,185,129,.8)', 'rgba(239,68,68,.8)', 'rgba(245,158,11,.8)'], borderColor: ['#10b981', '#ef4444', '#f59e0b'], borderWidth: 2, hoverOffset: 8 }] },
...
function renderWeekdayChart() {
  ...
  state.charts.weekday = new Chart(el.getContext('2d'), {
    ...
    options: chartDefaults({ y: { grid: { color: 'rgba(99,140,255,.06)' }, ticks: { color: '#64748b' } }, y1: { position: 'right', grid: { display: false }, ticks: { color: '#3b82f6', callback: v => v + '%' }, max: 100, min: 0 }, x: { grid: { display: false }, ticks: { color: '#64748b' } } })
  });
}
...
function renderAssetChart() {
  ...
  state.charts.asset = new Chart(el.getContext('2d'), {
    type: 'bar',
    data: { labels: top.map(a => a.asset), datasets: [{ label: 'P&L', data: top.map(a => a.pnl), backgroundColor: top.map(a => a.pnl >= 0 ? 'rgba(16,185,129,.7)' : 'rgba(239,68,68,.7)'), borderRadius: 6 }] },
...
function renderRRChart() {
  ...
  const bins = {};
  trades.forEach(t => { const b = Math.floor(t.rr * 2) / 2; bins[b] = (bins[b] || 0) + 1; });
  const sorted = Object.entries(bins).sort((a, b) => parseFloat(a[0]) - parseFloat(b[0]));
  state.charts.rr = new Chart(el.getContext('2d'), {
    type: 'bar',
    data: { labels: sorted.map(([k]) => k + 'R'), datasets: [{ data: sorted.map(([, v]) => v), backgroundColor: sorted.map(([k]) => parseFloat(k) >= 1 ? 'rgba(16,185,129,.7)' : 'rgba(239,68,68,.7)'), borderRadius: 4 }] },
    options: chartDefaults({ y: { grid: { color: 'rgba(99,140,255,.06)' }, ticks: { color: '#64748b' } }, x: { grid: { display: false }, ticks: { color: '#64748b' } } })
  });
}

function renderPublicEquityChart(equityCurve) {
  if (!equityCurve?.length) return;
  const el = document.getElementById('publicEquityChart');
  if (!el) return;
  const values = equityCurve.map(d => d.cumulative);
  const isPos = values[values.length - 1] >= 0;
  const ctx = el.getContext('2d');
  const grad = ctx.createLinearGradient(0, 0, 0, 280);
  grad.addColorStop(0, isPos ? 'rgba(34,197,94,.35)' : 'rgba(239,68,68,.35)');
  grad.addColorStop(1, 'rgba(0,0,0,0)');
  new Chart(ctx, {
    type: 'line',
    data: {
      labels: equityCurve.map(d => new Date(d.date).toLocaleDateString('fr-FR', { day: '2-digit', month: 'short' })),
      datasets: [{ data: values, borderColor: isPos ? '#22c55e' : '#ef4444', backgroundColor: grad, borderWidth: 2, fill: true, tension: 0.4, pointRadius: 0 }]
    },
    options: chartDefaults({ y: { grid: { color: 'rgba(255,255,255,.04)' }, ticks: { color: '#64748b', callback: v => '$' + v } }, x: { grid: { display: false }, ticks: { color: '#64748b', maxTicksLimit: 8 } } })
  });
}

function renderPublicMonthlyChart(monthly) {
  if (!monthly?.length) return;
  const el = document.getElementById('publicMonthlyChart');
  if (!el) return;
  const last12 = monthly.slice(-12);
  new Chart(el.getContext('2d'), {
    type: 'bar',
    data: { labels: last12.map(m => m.month), datasets: [{ data: last12.map(m => m.pnl), backgroundColor: last12.map(m => m.pnl >= 0 ? 'rgba(34,197,94,.7)' : 'rgba(239,68,68,.7)'), borderRadius: 6 }] },
    options: chartDefaults({ y: { grid: { color: 'rgba(255,255,255,.04)' }, ticks: { color: '#64748b', callback: v => '$' + v } }, x: { grid: { display: false }, ticks: { color: '#64748b' } } })
  });
}
